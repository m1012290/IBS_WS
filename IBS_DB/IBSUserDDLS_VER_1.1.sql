...
create table IBS_CUSTOMER_ENQUIRY(ENQUIRY_NO int,CUSTOMER_ID int,TYPE VARCHAR2(15),RECEIVED_DATE Timestamp,RCVD_FROM_CONTACT_ID int,PRODUCT_CLASS int,ENQUIRY_DESCRIPTION VARCHAR2(500),STATUS int,ENQUIRY_SUBJECTMATTER_EXPERT VARCHAR2(50),REC_CRE_USER_ID int,REC_CRE_DATE Timestamp,REC_UPD_USER_ID int,REC_UPD_DATE Timestamp,PRIMARY KEY (ENQUIRY_NO),FOREIGN KEY (CUSTOMER_ID) REFERENCES IBS_CUSTOMER(ID),FOREIGN KEY (RCVD_FROM_CONTACT_ID) REFERENCES IBS_CONTACT(ID),
...
create table IBS_UW_TRANSACTION_HEADER(ID int,POLICY_VERSION int,QUOTE_ID int,QUOTE_SLIP_ID int,QUOTE_SLIP_VERSION int,ENQUIRY_NO int,PRODUCT_CLASS int,SUB_CLASS VARCHAR2(50),CUSTOMER_ID int,INSURED_ID int,INSURED_NAME VARCHAR2(50),POLICY_START_DATE timestamp,POLICY_EXPIRY_DATE timestamp,
STATUS_CODE int,REC_CRE_USER_ID int,REC_CRE_DATE Timestamp,REC_UPD_USER_ID int,REC_UPD_DATE Timestamp,
PRIMARY KEY(ID,POLICY_VERSION),FOREIGN KEY (QUOTE_ID) REFERENCES IBS_QUOTE_COMPARISION_HEADER(ID),FOREIGN KEY (QUOTE_SLIP_ID,QUOTE_SLIP_VERSION) REFERENCES IBS_QUOTE_SLIP_HEADER(ID,QUOTE_SLIP_VERSION),FOREIGN KEY (PRODUCT_CLASS) REFERENCES IBS_PRODUCT_MASTER(CLASS),
FOREIGN KEY (STATUS_CODE) REFERENCES IBS_STATUS_MASTER(CODE));
