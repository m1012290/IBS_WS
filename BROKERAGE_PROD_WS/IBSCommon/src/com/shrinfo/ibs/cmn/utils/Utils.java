...
public class Utils{
    ...
    private static String appConfigPropertiesFile = "config/appconfig.properties";
    private static String errorMessagePropertiesFile = "config/messages.properties";
    ...
    /**
     * Returns an app config value split at ','. This can be used by all app config entries that can have multiple values
     * that don't use ',' in their values. In most cases, this kind of configuration works. However, if ',' is expected
     * to be part of the value itself, then consider using the overloaded method <code>getMultiValueAppConfig( String, String )</code>
    ...
    /**
     * Returns an app config value split at the specified <code>separator</code>. This can be used by all app config entries 
     * that can have multiple values that may have ',' in their values.
     * 
    ...
    public static String[] getMultiValueAppConfig( String key, String separator ){
        return Configurations.INSTANCE.getMultiValuedProperty( key, appConfigPropertiesFile, separator );
    }

    /**
     * Returns an app config value split at ',' and '#' and transformed into a Map. This can be used by all app config entries 
     * which will not have ',' or '#' in their values. This will work in most cases. However, if a different pair of separator
     * is required, consider using the overloaded method <code>getMultiValueAppConfigAsMap( String, String, String )</code>.
     * 
     * @param key The key for the property
     * @return
     */
     ...
     * 
     * @param key The key for the property
     * @param multiValueSeparator The multi-value separator that should be used to split the value configured into name-value pairs
     * @param nvPairSeparator The separator for the name-value pair
     * 
     * @return
     */
    public static Map<String, String>  getMultiValueAppConfigAsMap( String key, String multiValueSeparator, String nvPairSeparator ){
        
        Map<String, String> configAsMap = null;
        
        String[] multiValues = Configurations.INSTANCE.getMultiValuedProperty( key, appConfigPropertiesFile, multiValueSeparator );
     ...
     * 
     * @param context The Spring application context expected to have been loaded by the web container.
     */
    ...
    public static Object getBean( String beanName ){
        ...
        if( Utils.isEmpty( beanName ) ){
            throw new SystemException( CommonErrorKeys.UNKNOWN_ERROR, null, "BeanName should not be empty" );
        }
        //if( Utils.isEmpty( appContext ) || Utils.isEmpty( appContext.get() ) ){
        if( Utils.isEmpty( appContext ) ){
            throw new SystemException( CommonErrorKeys.UNKNOWN_ERROR, null, "applicationContext.xml is not loaded" );
        }
    ...
    /**
     * Loads the class specified the passed fully qualified name. The class is loaded using the ClassLoader that loaded Utils.class.
     * 
     * @param <T>
    ...
    public static <T> Class<T> loadClass( String className ){
        ...
        try{
            ...
            throw new SystemException( CommonErrorKeys.UNKNOWN_ERROR, e, "Class with name [", className, "] not found" );
        }
    ...
    public static Object newInstance( String classNameAsString, int numElems ) throws SystemException{
        ...
        try{
            ...
            throw new SystemException( CommonErrorKeys.CLASS_NOT_FOUND, e, classNameAsString, " is not a qualified class name" );
        }
        return instance;
    }
    ...
    public static <T> T newInstance( Class<T> clazz, Object... constructorArgs ){
        T instance = null;
        try{
            instance = clazz.getConstructor( getClassArrayForObjects( constructorArgs ) ).newInstance( constructorArgs );
        }
        catch( Exception e ){
            if( logger.isError() ) logger.error( e, "Error while trying to instantiate class [", clazz.getName(), "]" );
        }
    ...
    private static Class[] getClassArrayForObjects( Object... objects ){
        if( Utils.isEmpty( objects ) ) return null;
    ...
    /**
     * This method returns an actual parametrized type from the passed class, which is the child
     * of a generic class. <code>index</code> refers to the position in the order in which the 
     * generic types were declared in the class.
     *  
    ...
    public static <T> Class<?> getParametrizedType( Class<T> genericClass, int index ){
        ...
        try{
            ...
            if( logger.isError() ) logger.error( e, "Exception when looking for actual types in the generic class" );
        }
    ...
    /**
     * Checks if the passed class is abstract. Returns <code>true</code>, if yes; <code>false</code>, otherwise. Please note that it 
     * will return <code>true</code> even for interfaces.
     * @param clazz
     * @return
     */
    public static boolean isClassAbstract( Class<?> clazz ){
        if( Utils.isEmpty( clazz ) ) return false;
        return clazz.getModifiers() >= java.lang.reflect.Modifier.ABSTRACT ? true : false;
    }
    
    /**
     * A convenience method to invoke a method through reflection. Catches all the JDK exceptions and throws SystemException.
     * 
    ...
    public static Object invokeMethod( Method method, Object onWhat, Object... args ){
        try{
            ...
            throw new SystemException( CommonErrorKeys.UNKNOWN_ERROR, e, "IllegalArgumentException: Could not invoke method [", method.getName(), "] on instance of [", onWhat.getClass().getName(), "]" );
        }
        catch( IllegalAccessException e ){
            throw new SystemException( CommonErrorKeys.UNKNOWN_ERROR, e, "IllegalAccessException: Could not invoke method [", method.getName(), "] on instance of [", onWhat.getClass().getName(), "]" );
        }
        catch( InvocationTargetException e ){
            throw new SystemException( CommonErrorKeys.UNKNOWN_ERROR, e, "InvocationTargetException: Could not invoke method [", method.getName(), "] on instance of [", onWhat.getClass().getName(), "]" );
        }
    }
